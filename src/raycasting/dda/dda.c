...
/*   Updated: 2024/09/07 11:13:50 by rbutzke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 	converted from long double to integer. 
...
static void	dda_variables(t_dda *dda, t_data *data, t_ray *ray)
{
	...
	if (ray->ray_dir[Y] == 0)
		...
		dda->delta_dist[Y] = fabsl(1 / ray->ray_dir[Y]);
	if (ray->ray_dir[X] == 0)
		dda->delta_dist[X] = INFINITY;
	else
		dda->delta_dist[X] = fabsl(1 / ray->ray_dir[X]);
}
