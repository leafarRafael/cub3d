...
/*   Updated: 2024/09/07 11:15:44 by rbutzke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_direction(long double dir[2], char c);
static void	set_vector_rotation_cam_plane(long double plane[2],
				long double dir[2]);
...
static void	set_coords(t_data *data)
{
	...
	while (map[y])
	{
		...
		while (map[y][x])
		{
			if (is_coord(map[y][x]))
			{
				data->coord->pos[X] = (long double)x + 0.5;
				data->coord->pos[Y] = (long double)y + 0.5;
